...
async function buscarLeadsNoLinkedIn(niche, city, limit = 150, onProgress = null) {
    ...
    log(`Iniciando busca LinkedIn (Humana): ${niche} em ${city}`);
    ...
    try {
        ...
        const query = encodeURIComponent(`LinkedIn de Sócio ${niche} em ${city} whatsapp 9`);
        ...
        for (let p = 0; p < 2 && leads.length < limit; p++) {
            ...
            for (const item of results) {
                ...
                if (whatsapp && !processed.has(whatsapp) && validarDDD(whatsapp, dddsValidos)) {
                    ...
                    log(`[✓] Lead LinkedIn: ${item.nome} (${whatsapp})`);
                }
            }
