...
async function buscarLeadsNoInstagram(niche, city, limit = 150, onProgress = null) {
    ...
    log(`Iniciando busca Instagram (Humana): ${niche} em ${city}`);
    ...
    try {
        ...
        // Query humana para evitar bloqueios
        const query = encodeURIComponent(`Instagram de ${niche} em ${city} whatsapp 9`);
        ...
        for (let p = 0; p < 2 && leads.length < limit; p++) {
            const results = await page.evaluate(() => {
                ...
                document.querySelectorAll('.b_algo').forEach(el => {
                    ...
                    if (titleEl) {
                        ...
                        // Regex flexível: (XX) 9XXXX-XXXX ou XX9XXXXXXXX
            ...
            for (const item of results) {
                ...
                if (whatsapp && !processed.has(whatsapp) && validarDDD(whatsapp, dddsValidos)) {
                    ...
                    log(`[✓] Lead Instagram: ${item.nome} (${whatsapp})`);
                }
            }
